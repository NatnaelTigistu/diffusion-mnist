...
git init
git add .
git commit -m "Initial commit: DDPM implementation for MNIST"
```
...
```bash
git branch -M main
git remote add origin https://github.com/<your-username>/diffusion-mnist.git
...
4. Run all cells. This clones your repo, installs dependencies, trains for
   10 epochs (~5-10 min on a T4), and generates a sample grid.
...
[GitHub Personal Access Token](https://github.com/settings/tokens) since
Colab can't use your local git credentials).
...
[Git LFS](https://git-lfs.com/) or just link to it in the README (e.g.
uploaded to Google Drive).

## Everyday git workflow

Once the repo exists, the loop is:

```bash
# make changes locally (e.g. tweak unet.py)
git add src/unet.py
git commit -m "Increase UNet width to 64 base channels"
git push
```

```bash
# in Colab, pull the latest before training
%cd diffusion-mnist
!git pull
%cd src
!python train.py
...
For anything beyond quick experiments, branch off `main` so your working
version stays clean:

```bash
git checkout -b experiment/deeper-unet
# ... make changes, commit ...
git push -u origin experiment/deeper-unet
# open a Pull Request on GitHub, merge into main when it works
```
